...
export const createEpisodeRepository = (db: DrizzleDb): EpisodeRepository => ({
    findById: (id: number) => db.query.episodes.findFirst({ where: eq(episodes.id, id) }),
    ...
    insert: (data: { id: number; drawDate?: string; isDrawn: boolean }) => db.insert(episodes).values(data).onConflictDoNothing(),

    markDrawn: (id: number, drawDate: string) => db.update(episodes).set({ isDrawn: true, drawDate }).where(eq(episodes.id, id)),

    insertMany: (data: { id: number; drawDate?: string; isDrawn: boolean }[]) => db.insert(episodes).values(data).onConflictDoNothing(),
})
