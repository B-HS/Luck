...
const getDrawDate = (episodeId: number) => {
    const date = new Date(FIRST_DRAW_DATE.getTime() + (episodeId - 1) * MS_PER_WEEK)
...
const getLatestEpisode = () => {
    ...
    const drawDayThisWeek = new Date(FIRST_DRAW_DATE.getTime() + weeks * MS_PER_WEEK)
    const kstHour = kstNow.getUTCHours()
    const isPastDraw = kstNow >= drawDayThisWeek && (kstNow.getTime() > drawDayThisWeek.getTime() + 24 * 60 * 60 * 1000 || kstHour >= 22)
