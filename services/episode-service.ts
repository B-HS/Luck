...
export const createEpisodeService = (deps: {
  ...
  getLatestEpisodeNumber: () => {
    ...
    const drawDateThisWeek = new Date(
    ...
    const isPastDraw =
      now >= drawDateThisWeek &&
      (now.getTime() > drawDateThisWeek.getTime() + 24 * 60 * 60 * 1000 ||
        kstHour >= 22);
  ...
  getEpisodes: (limit = 20, offset = 0) =>
    deps.episodeRepo.findAll(limit, offset),
