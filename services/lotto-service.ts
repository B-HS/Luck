...
export const createLottoService = (deps: {
    ...
    const inFlight = new Map<number, Promise<LottoResult | null>>()

    const fetchAndStore = async (episode: number): Promise<LottoResult | null> => {
    ...
    return {
        getResult: async (episode: number) => {
            ...
            const pending = inFlight.get(episode)
            if (pending) {
                return pending
            }

            try {
                const promise = fetchAndStore(episode)
                inFlight.set(episode, promise)
